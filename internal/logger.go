...
func GetLogger() *Logger {
	if logger == nil {
		log.Printf("Logger not initialized, fallback to stderr.")
...
func (l *Logger) log(level, msg string, context map[string]interface{}) {
	...
	if l.logFile != os.Stderr && l.logFile != os.Stdout {
		l.logFile.Sync()
	}

	//TODO: move it to another log file that has a "prettier" postfix.
