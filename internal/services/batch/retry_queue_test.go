...
func TestRetryQueue_Add(t *testing.T) {
	...
	queue.Add(FailedRecord{OriginalOutput: `{"id": 1}`, Error: llm.NewLLmCallError(llm.InvalidFormat, fmt.Errorf("parse error"))})
	queue.Add(FailedRecord{OriginalOutput: `{"id": 2}`, Error: llm.NewLLmCallError(llm.InvalidFormat, fmt.Errorf("parse error"))})
...
func TestRetryQueue_FailedCount(t *testing.T) {
	...
	queue.Add(FailedRecord{OriginalOutput: `{"id": 1}`, Error: llm.NewLLmCallError(llm.Unknown, nil)})
